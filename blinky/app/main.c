...
volatile STATE CurrentState = STATE_ERROR;
...
int main(void){
    ...
    char message[256];
    while(1){
    Timer_Delay_Ticks_NoneBLocking(100000000);
    
    if(CurrentState == STATE_IDLE){
        CurrentState = HandleEventDS18B20(CurrentState);
    }
    else if(CurrentState != STATE_READ_DONE && CurrentState != STATE_ERROR){
        CurrentState = HandleEventDS18B20(CurrentState);
    }
    else if(CurrentState == STATE_READ_DONE){
        ...
        USB_UART_SendMessage("DS18B20 ERROR\n", 14);
        CurrentState = STATE_IDLE;
    }
}
    return 0;
...
STATE HandleEventDS18B20(STATE curState){
    volatile STATE currentState = curState;
    switch (currentState)
    {
    case STATE_IDLE:
    ...
    case STATE_MEASURING:
        ...
        DS18B20_WriteByte(0xCC);
        DS18B20_WriteByte(0x44);
        currentState = STATE_WAIT_MEASURING;
        start = msticks;
        break;
